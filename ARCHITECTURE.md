...
| 5 | **Disband / refund stranding.** A pre-audit red-team found this was actually **Critical**: when ≥2 *trailing* recipients default, `_payout`'s skip loop advances `currentRound` past the round the survivors contributed to, and `claimRefund` keyed its stuck-contribution recovery off the (advanced) `currentRound` → honest contributions stranded with no withdrawal path. | **Resolved (red-team pass).** `_payout` now records `cancelledRound` (the round captured *before* the skip loop) on cancel, and `claimRefund` refunds against it — balance-exact across full-wipeout, last-slot, and ≥2-trailing-default cancels (regression test `test_TrailingDefaults_NoStranding`). Residual: full *lifetime* net-position accounting across many partial rounds is deliberate v2 (the early-paid keep their pots; the defaulter forfeits — correct ROSCA economics, see DESIGN doc). | — |
